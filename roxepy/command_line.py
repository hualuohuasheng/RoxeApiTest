import argparse
from .clroxe import Clroxe
from .testroxe import TestRoxe
from .utils import parse_key_file
from .exceptions import InvalidPermissionFormat, ROXESetSameAbi, ROXESetSameCode
import json
...
def set_code(ce, account, permission, code, key, broadcast, timeout):
    ...
    except ROXESetSameCode:
        print('Skipping set code because the new code is the same as the existing code')


def clroxe():
    parser = argparse.ArgumentParser(description='Command Line Interface to ROXE via python')
    parser.add_argument('--api-version', '-v', type=str, default='v1', action='store', dest='api_version')
    parser.add_argument('--url', '-u', type=str, action='store', default='https://proxy.roxenode.tools', dest='url')
    ...
    system_subparsers = system_parser.add_subparsers(dest='system', help='Send roxe.system contract action to the blockchain')
    ...
    newacct_parser.add_argument('--stake-net', type=str, action='store', default='1.0000 ROC', dest='stake_net')
    newacct_parser.add_argument('--stake-cpu', type=str, action='store', default='1.0000 ROC', dest='stake_cpu')
    ...
    #
    # connect
    ce = Clroxe(url=args.url, version=args.api_version)
...
def testroxe():
    parser = argparse.ArgumentParser(description='ROXE testing harness')
