...
from .keys import ROXEKey
from .signer import Signer
from .utils import sig_digest, sha256
from .types import ROXEEncoder, Transaction, PackedTransaction, Abi
from .exceptions import (ROXEKeyError, ROXEMsigInvalidProposal, ROXESetSameAbi, ROXESetSameCode)
import json
...
class Clroxe:
    ...
    def get_actions(self, acct_name, pos=-1, offset=-20, timeout=30):
        ...
        {"account_name":"roxenewyorkio","pos":-1,"offset":-20}
        """
        p_json = {'account_name': acct_name, "pos": pos, "offset": offset}
        return self.post('history.get_actions', params=None, json=p_json, timeout=timeout)

    def get_currency(self, code='roxe.token', symbol='ROC', timeout=30):
        """
        POST /v1/chain/get_currency_stats HTTP/1.0
        {"json":false,"code":"roxe.token","symbol":"ROC"}
        """
        p_json = {'json': False, 'code': code, 'symbol': symbol}
        return self.post('chain.get_currency_stats', params=None, json=p_json, timeout=timeout)

    def get_currency_balance(self, account, code='roxe.token', symbol='ROC', timeout=30):
        """
        POST /v1/chain/get_currency_balance HTTP/1.0
        {"account":"roxe","code":"roxe.token","symbol":"ROC"}
        """
        p_json = {'account': account, 'code': code, 'symbol': symbol}
        return self.post('chain.get_currency_balance', params=None, json=p_json, timeout=timeout)
    ...
    def get_table(self, code, scope, table, index_position='', key_type='', lower_bound='', upper_bound='', limit=10, timeout=30):
        ...
        {"json":true,"code":"roxe","scope":"roxe","table":"producers","index_position":"","key_type":"name","lower_bound":"","upper_bound":"","limit":10}
        """
        p_json = {"json": True, "code": code, "scope": scope, "table": table, "key_type": key_type, "index_position": index_position, "lower_bound": lower_bound, "upper_bound": upper_bound, "limit": limit}
        return self.post('chain.get_table_rows', params=None, json=p_json, timeout=timeout)
    ...
    def push_transaction(self, transaction, keys, broadcast=True, compression="none", timeout=30):
        """
        parameter keys can be a list of WIF strings or ROXEKey objects or a filename to key file
        :param transaction:
        :param keys:
        :param broadcast:
        :param compression:
        ...
        if not isinstance(keys, list):
            if not isinstance(keys, Signer):
                raise ROXEKeyError('Must pass a class that extends the roxepy.Signer class')
            keys = [keys]
        ...
        data = json.dumps(final_trx, cls=ROXEEncoder)
        # print(data)
        if broadcast:
    ...
    def abi_bin_to_json(self, code, action, binargs, timeout=30):
        p_json = {'code': code, 'action': action, 'binargs': binargs}
        return self.post('chain.abi_bin_to_json', params=None, json=p_json, timeout=timeout)

    def abi_json_to_bin(self, code, action, args, timeout=30):
        p_json = {'code': code, 'action': action, 'args': args}
        return self.post('chain.abi_json_to_bin', params=None, json=p_json, timeout=timeout)
    ...
    def multisig_review(self, proposer, proposal):
        review = []
        prop = self.get_table(code="roxe.msig", scope=proposer, table="proposal", lower_bound=proposal, limit=1)
        if prop['rows']:
            ...
            raise ROXEMsigInvalidProposal("{} is not a valid proposal".format(proposal))
        #
        return review
    ...
    def create_account(self, creator, creator_privkey, acct_name, owner_key,
                       active_key='', stake_net='1.0000 ROC', stake_cpu='1.0000 ROC', ramkb=8, permission='active',
        ...
        newaccount_data = self.abi_json_to_bin('roxe', 'newaccount', {'creator': creator, 'name': acct_name, 'owner': owner_auth, 'active': active_auth})
        ...
        buyram_data = self.abi_json_to_bin('roxe', 'buyrambytes', {'payer': creator, 'receiver': acct_name, 'bytes': ramkb * 1024})
        buyram_json = {
            'account': 'roxe',
            'name': 'buyrambytes',
            'authorization': [
        ...
        # create delegatebw
        delegate_data = self.abi_json_to_bin('roxe', 'delegatebw',
