...
from .exceptions import ROXEBufferInvalidType, ROXEInvalidSchema, ROXEUnknownObj, ROXEAbiProcessingError
import json
import binascii
import struct
import six
...
class ROXEEncoder(json.JSONEncoder):
    def default(self, o):
...
class BaseObject(object):
    def __init__(self, d):
        ...
        try:
            ...
            raise ROXEInvalidSchema('Unable to process schema for {}'.format(type(self)))
    ...
    def _encode_buffer(self, value):
        ''' '''
        return ROXEBuffer(value).encode()
...
class Asset:
    ...
    def _string_to_symbol(self):
        ...
        return ROXEBuffer(UInt64(rslt)).encode()

    def encode(self):
        ''' '''
        power = '1'.ljust(self.precision + len('1'), '0')
        amount = ROXEBuffer(UInt64(self.amount * UInt64(power))).encode()
...
class Abi(BaseObject):
    ...
    def get_action(self, name):
        ...
        for act in self.actions:
            if act.name == name:
                return act
        raise ROXEUnknownObj('{} is not a valid action for this contract'.format(name))
    ...
    def get_struct(self, name):
        ...
        for struct in self.structs:
            if struct.name == name:
                return struct
        raise ROXEUnknownObj('{} is not a valid struct for this contract'.format(name))
    ...
    def get_action_parameters(self, name):
        ...
        for field in struct.fields:
            ...
            if(f in self._abi_map):
                ...
                raise ROXEUnknownObj("{} is not a known abi type".format(field.type))
        return parameters
    ...
    def json_to_bin(self, name, data):
        ...
        bin_buffer = ''
        for field in data:
            # create ROXEBuffer with value as a type of field
            if isinstance(params[field], list):
                ...
                field_buffer = ROXEBuffer(arr)
            else:
                field_type = type(params[field])
                field_buffer = ROXEBuffer(field_type(data[field]))
...
class PackedTransaction:
    ...
    def decode_actions(self, buf):
        ...
        while cnt < length and length:
            ...
            # get abi information
            contract_abi = self._clroxe.get_abi(acct_name)
            ...
            if abi_act["type"] != action_name:
                raise ROXEAbiProcessingError("Error processing the {} action".format(action_name))
    ...
    def decode_context_actions(self, buf):
        ...
        if length > 0:
            raise NotImplementedError("Currently roxepy does not support context_free_actions")
        # get length of action array
    ...
    def decode_trx_extensions(self, buf):
        ...
        (length, ctx_buf) = self._decode_buffer(VarUInt(), buf)
        if length > 0:
            raise NotImplementedError("Currently roxepy does not support transaction extensions")
        # get length of action array
...
class ROXEBuffer:
    ...
    def decode(self, objType, buf=None):
        ...
        if isinstance(objType, UInt32):
            ...
            raise ROXEBufferInvalidType("Cannot decode type: {}".format(type(objType)))
    ...
    def encode(self, val=None):
        ...
        if (isinstance(val, Name) or
            ...
            raise ROXEBufferInvalidType('Cannot encode type: {}'.format(type(val)))
